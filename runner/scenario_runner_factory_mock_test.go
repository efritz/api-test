...
func NewMockScenarioRunnerFactory() *MockScenarioRunnerFactory {
	return &MockScenarioRunnerFactory{
		NewFunc: &ScenarioRunnerFactoryNewFunc{
			defaultHook: func(*config.Scenario, logging.Logger, *semaphore.Weighted) ScenarioRunner {
				return nil
			},
		},
	}
...
type ScenarioRunnerFactoryNewFunc struct {
	defaultHook func(*config.Scenario, logging.Logger, *semaphore.Weighted) ScenarioRunner
	hooks       []func(*config.Scenario, logging.Logger, *semaphore.Weighted) ScenarioRunner
...
func (m *MockScenarioRunnerFactory) New(v0 *config.Scenario, v1 logging.Logger, v2 *semaphore.Weighted) ScenarioRunner {
...
// is empty.
func (f *ScenarioRunnerFactoryNewFunc) SetDefaultHook(hook func(*config.Scenario, logging.Logger, *semaphore.Weighted) ScenarioRunner) {
	f.defaultHook = hook
}
...
func (f *ScenarioRunnerFactoryNewFunc) PushHook(hook func(*config.Scenario, logging.Logger, *semaphore.Weighted) ScenarioRunner) {
	f.mutex.Lock()
...
func (f *ScenarioRunnerFactoryNewFunc) SetDefaultReturn(r0 ScenarioRunner) {
	f.SetDefaultHook(func(*config.Scenario, logging.Logger, *semaphore.Weighted) ScenarioRunner {
		return r0
	})
}
...
func (f *ScenarioRunnerFactoryNewFunc) PushReturn(r0 ScenarioRunner) {
	f.PushHook(func(*config.Scenario, logging.Logger, *semaphore.Weighted) ScenarioRunner {
		return r0
	})
}

func (f *ScenarioRunnerFactoryNewFunc) nextHook() func(*config.Scenario, logging.Logger, *semaphore.Weighted) ScenarioRunner {
	f.mutex.Lock()
